...
def one_term(my_possible_picks, enemy_possible_picks, picks_all, picker):
    possible_pairs = []

    # 非首发开牌情况
    # case: picker=me, chose from my_possible_picks
    if picker == 'me':
        for previous_picks in picks_all:
            last_pick = previous_picks[-1]
            for picks in my_possible_picks:
                pick_list_to_update = previous_picks[:]
                if is_possible_picks(last_pick, picks):
                    pick_list_to_update.append(picks)
                else:
                    pick_list_to_update.append([])
                possible_pairs.append(pick_list_to_update)
